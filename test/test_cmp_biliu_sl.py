...
def test_compare_bs_average():
    ...
    exam_biliu_data = os.path.join(definitions.ROOT_DIR, 'hydromodel/example/biliu_rain_daily_datas')
    exam_sl_data = os.path.join(definitions.ROOT_DIR, 'hydromodel/example/songliao_exam_stas')
    ...
    voronoi_gdf = gpd.read_file(os.path.join(definitions.ROOT_DIR, 'hydromodel/example/biliuriver_shp/碧流河流域_副本.shp'))
    ...
    result = pd.DataFrame({'Date': test_range, 'Biliu': rain_aver_list_biliu, 'SL': rain_aver_list_sl})
